Shader "Custom/TriplanarIndirectGeometry"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        V2G PopulateGeometryInput(Attributes input)
        {
            V2G output = (V2G)0;
            Triangle curPt = triangleBuffer[input.id / 3];
        ...
        PackedVaryings PopulateVaryingsFromGeometry(V2G IN, float3 v, float3 n)
        {
            ...
            input.uv2 = IN.uv2;
           #if UNITY_ANY_INSTANCING_ENABLED
            input.instanceID = IN.instanceID;
           #endif

            input.positionOS = v;
            input.normalOS = n;
            float3 t1 = cross(n, float3(0, 0, 1));
            float3 t2 = cross(n, float3(0, 1, 1));
